...
interface ERC20{
    function transfer(address recipient,uint256) external returns(bool);
    ...
    address public owner;
     address public tokenAddress;
      uint256 public tokenSalePrice ;
       ...
       modifier onlyOwner(){
        require(msg.sender == owner,"Only contract owner can perform this action");_;
       ...
       function buyToken(uint256 _tokenAmount) public payable {
        require(msg.value == multiply(_tokenAmount,tokenSalePrice),"Insufficent Ether provided for the token puchase");
       ...
       function transferToOwner(uint256 _amount) external payable{
        require(msg.value >= _amount,"Insufficent funds sent");

        (bool success,) = owner.call{value: _amount}("");
        require(success,"Transfer failed");
       }

       function transferEther(address payable _receiver) external payable {
         require(msg.value >= 1,"Insufficent funds sent");

        (bool success,) = _receiver.call{value: msg.value}("");
        require(success,"Transfer failed");
       }

       function withdrawlAllTokens() public onlyOwner {
        ERC20 token=ERC20(tokenAddress);
